...
import { Download, MessageSquare, TrendingUp, ArrowLeft } from "lucide-react";
...
export default function DoctorTestResultDetailsPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6">
        <div className="flex items-center justify-between">
            <div>
                <h1 className="text-3xl font-bold font-headline">Lab Report: {testName}</h1>
                <p className="text-muted-foreground">
                    Patient: {patient.name} (ID: {patient.id})
                </p>
            </div>
            <Button variant="outline" asChild>
                <Link href={`/doctor/patient-chart/${patient.id}`}>
                    <ArrowLeft className="mr-2 h-4 w-4" /> Back to Patient Chart
                </Link>
            </Button>
        </div>
        ...
        <Card>
            <CardHeader>
                ...
                <CardDescription>Total Cholesterol for {patient.name} over the last 2 years.</CardDescription>
            </CardHeader>
            <CardContent>
